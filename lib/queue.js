...
class Queue extends Emitter {
  ...
  _iterRoundRobin() {
    ...
    this.bclient.smembers(this.toKey('roundrobin'),
      idPromise.defer());
  ...
  _getJobForRoundRobinKey(key) {
    const idPromise = helpers.deferred();
    this.bclient.rpoplpush(this.toKey('waiting_' + key), this.toKey('active'),
      idPromise.defer());
    ...
    return idPromise.then((jobId) => {
      ...
      this.bclient.lrem(this.toKey('waiting'), 0, jobId, lremPromise.defer())
  ...
  _clearJobForRoundRobin(job) {
    if(!job || !job.data || !job.data[this.settings.roundrobin]) return job
    const cleanPromise = helpers.deferred();
    var key = job.data[this.settings.roundrobin]
    //We use client instead of bclient that can be used on another
    this.client.lrem(this.toKey('waiting_' + key), 0, job.id, cleanPromise.defer())
    return cleanPromise.then(() => job)
  }
  ...
  _getNextJob() {
    ...
    if(this.settings.roundrobin) {
      return this._commandable(true)
              ...
              .then(job => job || this._waitForJob().then(job => this._clearJobForRoundRobin(job)))
    }
