...
class Queue extends Emitter {
  ...
  destroy(cb) {
    const promise = this._commandable().then((client) => {
      ...
      const args = ['id', 'jobs', 'stallBlock', 'stalling', 'waiting', 'active',
        'succeeded', 'failed', 'delayed', 'notified', 'roundrobin']
  ...
  removeJob(jobId, cb) {
    const promise = this._evalScript('removeJob', 8,
      this.toKey('succeeded'), this.toKey('failed'), this.toKey('waiting'),
      this.toKey('active'), this.toKey('stalling'), this.toKey('jobs'), this.toKey('notified'),
  ...
  _iterRoundRobin() {
    ...
    this.client.smembers(this.toKey('roundrobin'),
      idPromise.defer());


    return idPromise.then((roundRobinKeys) => {
      if(! roundRobinKeys || roundRobinKeys.length == 0)
        return
      return this._iterForRoundRobinKey(roundRobinKeys)
    }, (err) => {
  ...
  _removeCurrentIndex(keys, index) {
    keys.splice(index, 1)
    return keys
  }

  _iterForRoundRobinKey(keys) {
    if(keys.length == 0) {
      return
    }
    var key = keys[this.roundRobinIndex]
    if(key) {
    ...
    var index = this.roundRobinIndex
    return this._getJobForRoundRobinKey(key).then(job => job || this._iterForRoundRobinKey(this._removeCurrentIndex(keys, index -1)))
  }

  _getJobForRoundRobinKey(key) {
    const idPromise = helpers.deferred();
    this.client.rpoplpush(this.toKey('waiting_' + key), this.toKey('active'),
      idPromise.defer());
    ...
    return idPromise.then((jobId) => {
      ...
      const lremPromise = helpers.deferred();
      this.client.lrem(this.toKey('waiting'), 0, jobId, lremPromise.defer())
      ...
      // returned the job id.
      return lremPromise.then(() => Job.fromId(this, jobId));
  ...
  _waitForJobNotifiation() {
    const idPromise = helpers.deferred();
    this.bclient.brpoplpush(this.toKey('waiting'), this.toKey('notified'), 0,
      idPromise.defer());
  ...
  _getNextJob() {
    ...
    if(this.settings.roundrobin) {
      return this._commandable(true)
              .then(() => this._iterRoundRobin())
              .then(job => job || this._waitForJobNotifiation().then(job => this._getNextJob())) //To avoid running twice a job, _waitForJob is used to be notified but not to provide job
    }
  ...
  _finishJob(err, data, job) {
    ...
    const multi = this.client.multi()
      ...
      .lrem(this.toKey('notified'), 0, job.id)
      .srem(this.toKey('stalling'), job.id);

    if(this.settings.roundrobin !== undefined) {
      var value = job.data && job.data[this.settings.roundrobin]
      if(value !== undefined)
        multi.lrem(this.toKey('waiting'), 0, job.id);
    }
    ...
    if (err) {
      ...
      if (delay < 0) {
        ...
        if (delay === 0) {
          if(this.settings.roundrobin) {
            var rrval = job.data[this.settings.roundrobin]
            multi.lpush(this.toKey('waiting_' + rrval), job.id);
          } else {
  ...
  process(concurrency, handler) {
    ...
    this.checkStalledJobs(this.settings.stallInterval, jobTick);
  ...
  _doStalledJobCheck() {
    return this._evalScript('checkStalledJobs', 5, this.toKey('stallBlock'),
      this.toKey('stalling'), this.toKey('waiting'), this.toKey('active'), this.toKey('notified'),
        ...
        for (let jobId of stalled) {
          if (this.settings.roundrobin) {
            Job.fromId(this, jobId).then((job) => {
              if (job) {
                var rrval = job.data[this.settings.roundrobin]
                this.client.lpush(this.toKey('waiting_' + rrval), jobId)
              }
            })
          }
