...
class Queue extends Emitter {
  ...
  _iterForRoundRobinKey(keys) {
    ...
    return this._getJobForRoundRobinKey(key).then(job => job || this._iterForRoundRobinKey(this._removeCurrentIndex(keys)))
  }
  ...
  _clearJobForRoundRobin(job) {
    ...
    this.bclient.lrem(this.toKey('waiting_' + key), 0, job.id, cleanPromise.defer())
