...
class Job extends Emitter {
  ...
  save(cb) {
    ...
    var roundrobin = this.queue.settings.roundrobin
    if(roundrobin !== undefined && this.data[roundrobin] !== undefined) {
      promise.then(jobId => {
        var rrval = this.data[roundrobin]
        this.queue._commandable().then((client) => {
            const saveRRval = helpers.deferred();
            const multi = client.multi()
            multi.sadd(toKey("roundrobin"), rrval)
            multi.lpush(toKey('waiting_') + rrval, jobId)
            multi.exec(saveRRval.defer());
            return saveRRval;
        });
        return jobId
      })
